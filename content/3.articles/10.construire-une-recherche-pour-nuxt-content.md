...
Avant même de s'attaquer à créer une barre de recherche, comprenons la logique de ce que nous allons faire. En effet, il est important de comprendre le fonctionnement pour pouvoir ensuite l'adapter aux besoins du site ou s'il y a besoin d'utiliser un autre outil.
...
Mais nous allons voir pour aller plus loin et optimiser les performances.
...
La dernière chose à faire est d'indique à Nitro qu'il doit pré-rendre ce fichier texte lors de la création du site web. Pour cela, direction `nuxt.config.ts`, où nous allons ajouter une règle sur la route `/api/search` :
...
La partie la plus complexe est passé et la création de la recherche est maintenant très simple.
