...
L'objectif de cet article est de présenter cet outil ! En effet, il est utilisé dans d'autres projets comme [c12](https://github.com/unjs/c12).
...
Maintenant que l'on a pris le temps de comprendre la problématique, réfléchissons à une solution.
...
function merge(defaultConfig, userConfig) {
  ...
  // Pour chaque clé, on va merger les valeurs
  for (const key of keys) {
    ...
    // Si la valeur par défaut ou la valeur utilisateur est un objet, on va merger les 2 objets.
    // On fait cela de manière récursive pour merger les objets en profondeur et éviter la problématique de la solution naïve.
    if (typeof defaultValue === 'object' || typeof userValue === 'object')
      mergedConfig[key] = merge(defaultValue, userValue)

    // Si ce n'est pas un objet, on prend la valeur de l'utilisateur si elle existe, sinon on prend la valeur par défaut.
    else
...
<!-- faire très simple, genre 1 ligne -->
<!-- bien inscister sur l'ordre (object, default), plus tu est à gauche, plus de as de poids -->

### Un merge personnalisable

<!-- reprendre un peu l'exemple du readme -->

<!-- évoquer rapidement defuFn et defuArrayFn en évoquant via une simple phrase ce que ça fait (mais pas besoin de beaucoup plus comme je n'ai pas de cas d'usage sous la main) -->
