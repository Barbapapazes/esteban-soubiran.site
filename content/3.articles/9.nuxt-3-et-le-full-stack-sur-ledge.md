...
_J'aime Nuxt et cet article se veut être un **état de l'art**. Il se veut constructif et bienveillant. Devenir grand prend du temps et ce qui été accompli ces dernières années est déjà merveilleux ! :yellow_heart:_
...
::alert{type="info"}
Avec cette article, nous allons prendre l'exemple de Nuxt. Cependant, les autres frameworks cités sont dans la même situation et les conclusions de cet article sont valables pour eux.
::
...
Cependant, mettre en production une telle application n'est pas si simple puisqu'il faut avoir un serveur, le payer, le provisionner avec les bons outils, comme Nginx, Fail2ban, Redix et une base de données SQL, pour le bon fonctionnement de l'application et de l'infrastructure et mettre en place les pipelines de déploiement. C'est une tâche qui peut être très complexe, qui nécessite des compétences spécifiques et qui demande du temps. Ce temps utilisé à mettre en production l'application n'est pas celui utilisé pour **produire de la valeur pour l'utilisateur**.

## Les Changements Depuis Nuxt 2

Cet article n'aurait pas été possible, ou n'aurait eu que peu d'intérêt, il y a encore quelques années, avant le **lancement de Nuxt 3** et l'arrivé des **edge functions**.

### Nuxt 2

Nuxt, jusqu'à sa version 2, ne permettait pas d'interagir simplement avec un serveur et une base de données. Pour utiliser le serveur, il fallait mettre en place un [ "server middleware"](https://v2.nuxt.com/docs/configuration-glossary/configuration-servermiddleware). Du coup, faire du full-stack était extrêmement limité.
...
À partir de 2016, le mot [Jamstack est popularité](https://jamstack.org/) par Matt Biilmann, créateur de :icon{name="netlify"} Netlify, et ça change la manière de voir le web. Principalement, l'une des idées est de penser statique pour pouvoir stocker son site sur des CDN pour de meilleurs performances, une meilleure sécurité, améliorer l'expérience développeur et réduire les coûts.

::alert{type="info"}
En savoir plus sur la Jamstack sur [jamstack.wtf](https://jamstack.wtf/)
::

### L'Edge

En mars 2018, [Cloudflare annonce ses Workers](https://blog.cloudflare.com/introducing-cloudflare-workers/) permettant d'exécuter du code sur son CDN. C'est une révolution puisque cela permet de faire du serverless au plus proche des clients sans les désavantages des serverless functions. Ainsi, le CDN n'est plus limité aux fichiers statiques tout en gardant les avantages de la Jamstack à savoir le déploiement simplifié, les performances, la sécurité et les coûts réduits.

::detail{title="Serverless function vs Edge function"}
Une edge function est une serverless functions puisqu'elle s'exécute sur un serveur géré par un tier et est déclenché par une requête HTTP.

Une edge function a l'avantage d'avoir [un démarrage à froid presque instantané, de l'ordre de la milliseconde](https://mikhail.io/serverless/coldstarts/big3/) par rapport à une serverless function. De plus, elle s'exécute au plus proche du client, ce qui permet d'avoir des temps de réponse très faible.

Pour autant, une edge function ne peut pas faire plus de 1MB, _node\_modules inclus_ ce qui est très limité et demande une certaine rigueur dans le code qui est écrit. Aussi, les API disponibles au sein des edges functions sont limitées demandant souvent l'écriture d'un code spécifique.

Ainsi, l'adoption des edges functions sans être vendor lock-in est complexe.
::

Par la suite, Netlify a annoncé ses [serverless functions en avril 2021](https://www.netlify.com/blog/2021/04/19/announcing-native-typescript-support-for-netlify-functions/) et ses [edge functions en avril 2022](https://www.netlify.com/blog/announcing-serverless-compute-with-edge-functions/). Vercel a annoncé ses [edge functions en décembre 2022](https://vercel.com/blog/edge-functions-generally-available).

Ainsi, l'arrivé des edge functions simplifie la gestion de l'infrastructure et donne un argument aux frameworks front-end pour se lancer dans le full-stack et un avantage vis-à-vis des frameworks back-end.

Pour autant, et nous allons le voir ensemble, il reste encore beaucoup de chemin à parcourir pour que Nuxt soit un framework full-stack.
...
Cette application est très simplement réalisable avec un framework comme AdonisJS mais qu'en est-il avec Nuxt ?
...
Pour autant, nous allons voir par la suite que la remonté des problèmes de validations des données va être plus complexe avec Nuxt qu'avec AdonisJS.

### La Base de Données

Pour la base de données de production, nous avons choisi D1. En développement, nous allons devoir trouver une alternative locale. Pour cela, nous allons utiliser SQLite puisque c'est là dessus qu'est basé D1.
...
La configuration de Drizzle n'est pas très complexe mais elle demande du temps et n'est, pour le moment, pas du tout intégré à Nitro. Il faut donc trouver le moyen de gluer Drizzle à Nitro.
...
Pourtant, c'est une action assez courante dans le développement et l'évolution d'une application. Je pense que c'est point important surtout qu'en déployant dans l'edge, il est question de déployer rapidement et fréquemment. Alors si la base de données devient une contrainte au bout du second déploiement parce qu'il y a besoin de mettre à jour le schéma, c'est un problème.
...
La conséquence de ça, c'est une mauvaise intégration et gestion des erreurs renvoyées au client. Supposons que j'envoie au server un nom mais pas d'URL. Sa réponse est la suivante :
...
Le client peut alors faire un traitement pour afficher le bon message d'erreur mais rien n'est prévu pour ça. Je vais devoir créer moi-même un système de gestion des erreurs pour afficher le bon message d'erreur à l'utilisateur sur le bon champ du formulaire. Depuis, si je veux personnaliser le message renvoyé parle serveur, je vais devoir réécrire une bonne partie de la gestion des erreurs.

### L'Authentification

Sur cette section, rien n'est prévu ni par Nuxt ni par Nitro pour gérer ses utilisateurs à la main et il faudrait donc soit écrire à la main un système d’authentification soit la gestion d'oauth avec un provider comme Google ou GitHub. C'est un travail qui peut être long et complexe.
